...
const CategoryMenuScreen = (props) => {
  ...
  const mealTile = ({item}) => {
    return ( 
        <MealItem title={item.title} imageUrl={item.imageUrl} onPress={() => {props.navigation.navigate("MenuDetail",{categoryColor: props.navigation.getParam("color"),title: item.title})}} />
    )
  }
